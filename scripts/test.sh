...
INSECURE_SERVER="127.0.0.1:8081"
...
mb::test::user()
{
  ...
  # 1. 如果有 setcreed、posttest 用户先清空
  ${DCURL} "${token}" http://${INSECURE_SERVER}/v1/users/setcreed > /dev/null
  ${DCURL} "${token}" http://${INSECURE_SERVER}/v1/users/posttest > /dev/null

  # 2. 创建 setcreed 用户
  ${CCURL} "${Header}" "${token}" http://${INSECURE_SERVER}/v1/users \
    -d'{"username":"setcreed","password":"miniblog1234","nickname":"belm","email":"nosbelm@qq.com","phone":"1818888xxxx"}' > /dev/null

  # 3. 列出所有用户
  ${RCURL} "${token}" "http://${INSECURE_SERVER}/v1/users?offset=0&limit=10" > /dev/null

  # 4. 获取 setcreed 用户的详细信息
  ${RCURL} "${token}" http://${INSECURE_SERVER}/v1/users/setcreed > /dev/null

  # 5. 修改 setcreed 用户
  ${UCURL} "${Header}" "${token}" http://${INSECURE_SERVER}/v1/users/setcreed \
    -d'{"nickname":"setcreed","email":"setcreed_modified@foxmail.com","phone":"1812884xxxx"}' > /dev/null
